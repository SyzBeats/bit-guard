...
const SecretMutation = {
	 ...
	 */
	async createMessage(_, args, ctx: Context): Promise<Message> {
		const { data } = args;
	 ...
	 */
	async deleteMessage(_, args, ctx: Context): Promise<Message> {
		const { data } = args;
		const { prisma, req } = ctx;
	 ...
	 * to any recipient.
	 */
	async createSignal(_, args, ctx: Context): Promise<ICreateSignalOutput> {
		const { data } = args;
		const { prisma, req } = ctx;
	 ...
	 */
	async deleteSignal(_, args, ctx: Context): Promise<Signal | null> {
		const { data } = args;
		const { prisma, req } = ctx;
	 ...
	 * to be authenticated.
	 */
	async createPublicSignal(_, args, ctx: Context): Promise<ICreateSignalOutput> {
		const { data } = args;
		const { prisma } = ctx;
